...
def load_config():
    ...
    if not config.read(CONFIG_FILE):
        ...
        config['Detection'] = {'ROI_X1': str(DEFAULT_ROI_X1), 'ROI_Y1': str(DEFAULT_ROI_Y1), 'ROI_X2': str(DEFAULT_ROI_X2), 'ROI_Y2': str(DEFAULT_ROI_Y2), 'HEX_GREY': DEFAULT_HEX_GREY, 'HEX_WHITE': DEFAULT_HEX_WHITE, 'COLOR_TOLERANCE': str(DEFAULT_COLOR_TOLERANCE), 'MIDDLE_THRESHOLD': str(DEFAULT_MIDDLE_THRESHOLD), 'BAR_THICKNESS_PERCENTAGE': str(DEFAULT_BAR_THICKNESS_PERCENTAGE), 'WHITE_AREA_WIDTH_INCREASE': str(DEFAULT_WHITE_AREA_WIDTH_INCREASE), 'GREY_LINE_MIN_AREA': str(DEFAULT_GREY_LINE_MIN_AREA)}
        config['Automation'] = {'CLICK_COOLDOWN_DURATION': str(DEFAULT_CLICK_COOLDOWN_DURATION), 'PREDICTION_ENABLED': str(DEFAULT_PREDICTION_ENABLED), 'LOOKAHEAD_FACTOR': str(DEFAULT_LOOKAHEAD_FACTOR), 'EXPONENTIAL_POWER': str(DEFAULT_EXPONENTIAL_POWER)}
    ...
    return {'ROI_X1': roi_x1, 'ROI_Y1': roi_y1, 'ROI_X2': roi_x2, 'ROI_Y2': roi_y2, 'HEX_GREY': hex_grey, 'HEX_WHITE': hex_white, 'COLOR_TOLERANCE': color_tolerance, 'MIDDLE_THRESHOLD': middle_threshold, 'CLICK_COOLDOWN_DURATION': click_cooldown_duration, 'BAR_THICKNESS_PERCENTAGE': bar_thickness_percentage, 'WHITE_AREA_WIDTH_INCREASE': white_area_width_increase, 'GREY_LINE_MIN_AREA': grey_line_min_area, 'PREDICTION_ENABLED': prediction_enabled, 'LOOKAHEAD_FACTOR': lookahead_factor, 'EXPONENTIAL_POWER': exponential_power}
    ...
    while running:
        try:
            ...
            if grey_center and white_center:
                ...
                grey_line_end = (int(arc_center[0] + radius * np.cos(angle_grey_rad)), int(arc_center[1] + radius * np.sin(angle_grey_rad)))
                cv2.line(display_image_bgr, arc_center, grey_line_end, (255, 0, 0), 1)
                white_line_end = (int(arc_center[0] + radius * np.cos(angle_white_rad)), int(arc_center[1] + radius * np.sin(angle_white_rad)))
                ...
                cv2.ellipse(display_image_bgr, arc_center, (int(radius), int(radius)), 0, start_angle, end_angle, (255, 255, 255), 2)
            else:
                current_arc_velocity = 0.0
                last_arc_distance = None
                last_grey_time = None
